...
void debug::socketCommands(int &type, int &val){
	int temp1 = 0, temp2 = 0;
	while(1){	
		usleep(10000);
...
void picam::process(){	
    ...
    while (1) {
		...
		if(!camshift){
			calcHist(&processImages[2], 1, channels, processImages[8], roi_hist, 1, histSize, range);
			normalize(roi_hist, roi_hist, 0, 255, NORM_MINMAX);
		}
		else{
			Mat dst;
			calcBackProject(&processImages[2], 1, 0, channels, roi_hist, dst, range);
...
void picam::streaming(){
	...
	thread thread_display(&debug::socketDisplay, db, ref(showMtx), ref(showImage));
	thread thread_commands(&debug::socketCommands, db, ref(type), ref(val));
